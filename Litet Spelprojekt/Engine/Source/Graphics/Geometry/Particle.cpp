...
void Particle::Create()
{
	...
	//Normal
	GL_CALL(glVertexAttribPointer(1, 3, GL_FLOAT, GL_TRUE, sizeof(ParticleVertex), (void*)(3 * sizeof(float))));
	GL_CALL(glEnableVertexAttribArray(1));
	//TexCoords
	GL_CALL(glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(ParticleVertex), (void*)(6 * sizeof(float))));
	...
	for (uint32 i = 0; i < NumInstances; i++)
	{
		...
		instances[i].Model = glm::mat4(1.0f);
	}
	...
	//Per instance color
	GL_CALL(glVertexAttribPointer(3, 4, GL_FLOAT, GL_FALSE, sizeof(ParticleInstance), (void*)0));
	GL_CALL(glEnableVertexAttribArray(3));
	GL_CALL(glVertexAttribDivisor(3, 1));

	//Instance model matrix
	GL_CALL(glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(ParticleInstance), (void*)sizeof(glm::vec4)));
	GL_CALL(glEnableVertexAttribArray(4));
	GL_CALL(glVertexAttribDivisor(4, 1));
	GL_CALL(glVertexAttribPointer(5, 4, GL_FLOAT, GL_FALSE, sizeof(ParticleInstance), (void*)(2 * sizeof(glm::vec4))));
	GL_CALL(glEnableVertexAttribArray(5));
	GL_CALL(glVertexAttribDivisor(5, 1));
	GL_CALL(glVertexAttribPointer(6, 4, GL_FLOAT, GL_FALSE, sizeof(ParticleInstance), (void*)(3 * sizeof(glm::vec4))));
	GL_CALL(glEnableVertexAttribArray(6));
	GL_CALL(glVertexAttribDivisor(6, 1));
	GL_CALL(glVertexAttribPointer(7, 4, GL_FLOAT, GL_FALSE, sizeof(ParticleInstance), (void*)(4 * sizeof(glm::vec4))));
