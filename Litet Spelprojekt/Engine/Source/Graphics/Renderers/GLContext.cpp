...
void APIENTRY glDebugOutput(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* pMessage, const void* pUserParam);
...
{
	if (pFramebuffer == m_pCurrentFramebuffer)
	{
		return;
	}
...
void APIENTRY glDebugOutput(GLenum source, GLenum type, GLuint id, GLenum severity, GLsizei length, const GLchar* pMessage, const void* pUserParam)
{
	...
	std::cout << "---------------" << std::endl;
	std::cout << "Debug message (" << id << "): " << pMessage << std::endl;
