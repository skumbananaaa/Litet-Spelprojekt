	...
	m_SelectionHandler(false)
{
	...
	m_Panels.push_back(CreateExpandable("Sjukvårdare", x, y, width, (float)buttonHeight, TEXTURE::ICON_SKILL_MEDIC, MEDIC, crew, buttonColor, textOffset));
	m_Panels.push_back(CreateExpandable("Rökdykare", x, y, width, (float)buttonHeight, TEXTURE::ICON_SKILL_FIRE, SMOKE_DIVER, crew, buttonColor, textOffset));

	for (uint32 i = (uint32)m_Panels.size(); i < crew->NrOfSquads(); i++)
	{
		m_Panels.push_back(CreateExpandable("Grupp " + std::to_string(i - 1), x, y, width, (float)buttonHeight, TEXTURE::ICON_SKILL_STRENGTH, i, crew, buttonColor, textOffset));
	}
...
{
	Game::GetGame()->m_pSceneGame->GetUICrewMember()->SetCrewMember(Game::GetGame()->m_pSceneGame->GetCrewmember((uint32)reinterpret_cast<uint64>(((Button*)selection)->GetUserData())));
}
...
{
	context->RenderTexture(ResourceHandler::GetTexture2D((uint32)reinterpret_cast<uint64>(object->GetUserData())), object->GetWidth() - 40, 10, 30, 30, GUIContext::COLOR_WHITE);
}

void UICrew::OnButtonPressed(Button* button)
{
	ProgressButton* progressButton = (ProgressButton*)button;
	progressButton->StartAnimation((float)Random::GenerateInt(3, 15));
...
PanelExpandable* UICrew::CreateExpandable(const std::string& text, float x, float y, float width, float buttonHeight, uint32 icon, int index, Crew* crew, const glm::vec4& buttonColor, const glm::vec2& textOffset)
{
	...
	panel->SetUserData(reinterpret_cast<void*>((uint64)icon));
	...
	for (uint32 i = 0; i < crew->GetSquadCount(index); i++)
	{
