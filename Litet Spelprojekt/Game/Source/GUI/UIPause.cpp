...
UIPause::UIPause(float x, float y, float width, float height) : Panel(x, y, width, height), 
	...
	m_pOptionsPanel(nullptr),
	m_pTutorialPanel(nullptr)
{
...
void UIPause::CreateOptionsPanel() noexcept
{
	m_pOptionsPanel = new Panel((GetWidth() - 600) / 2, (GetHeight() - 600) / 2, 600, 600);
...
void UIPause::CreateTutorialPanel() noexcept
{
	m_TutorialScreenIndex = 0;
	...
	m_pTutorialPanel = new Panel(0, 0, GetWidth(), GetHeight());
	m_pButtonNext = new Button(GetWidth() - 311, 45, 200, 100, "Nästa");
...
void UIPause::CreatePausePanel() noexcept
{
	m_pMenuPanel = new Panel((GetWidth() - 600) / 2, (GetHeight() - 600) / 2, 600, 600);
...
void UIPause::Clear() noexcept
{
	...
	if (m_pOptionsPanel)
	{
		Remove(m_pOptionsPanel);
		m_pOptionsPanel = nullptr;
	}
	if (m_pTutorialPanel)
	{
		Remove(m_pTutorialPanel);
		m_pTutorialPanel = nullptr;
	}
}
