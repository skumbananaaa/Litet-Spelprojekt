...
UISelectedCrew::UISelectedCrew(const std::string& name) : ProgressButton(0.0f, 0.0f, 50.0f + name.length() * 10.0f, 25.0f, name)
{
...
void UISelectedCrew::UpdatePosition(const glm::vec3 & position) noexcept
{
	...
	crewWorldPos.y = glm::clamp(crewWorldPos.y, std::floor(position.y), std::floor(position.y / 2.0f) * 2.0f + 1.9f);
	crewWorldPos.x += Game::GetGame()->m_pSceneGame->GetExtension() * glm::floor(crewWorldPos.y / 2.0f);
	glm::vec4 pos = camera.GetCombinedMatrix() * glm::vec4(crewWorldPos, 1.0f);
	pos = pos / pos.w;
	float halfWidth = Game::GetGame()->GetWindow().GetWidth() / 2.0f;
	float halfHeight = Game::GetGame()->GetWindow().GetHeight() / 2.0f;
	float x = pos.x * halfWidth + halfWidth - GetWidth() / 2.0f;
	float y = pos.y * halfHeight + halfHeight + GetHeight() / 2.0f;
	SetPosition(x, y);
}
