...
{
	float map[LEVEL_SIZE_X * LEVEL_SIZE_Y * LEVEL_SIZE_Z];
	int concealed;
	uint roomId;
};
...
void main()
{
	...
	{
		//if (concealed == 0 || map[mapPos.x][mapPos.y][mapPos.z] == roomId || position.y >= 5.9f || position.x > 10.5f || position.x < 0.5f || position.z > 40.5f || position.z < 0.5f)
		{
	...
	c = vec3(map[1 * 252 + 4 * 42 + 1], map[1 * 252 + 4 * 42 + 7], map[6 * 252 + 4 * 42 + 7]) / 10.0f;
